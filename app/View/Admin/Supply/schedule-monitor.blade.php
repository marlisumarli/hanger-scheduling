...
@section('content')
    ...
    <div class="mb-4">
        <h1>SCHEDULE SUPPLY {{strtoupper($type)}}</h1>
    </div>
    <div class="row">

        @if($schedule_weeks->findScheduleSupplyId((strtolower($dateNow->format('YF').'-'.$type))) !== null)

            @foreach($schedule_m_categories as $category)
                    ...
                    <div class="card rounded-3 shadow-lg">
                        ...
                        <div class="card-body p-3">
                            <div class="row text-center">
                                ...
                                @foreach($schedule_weeks->findScheduleSupplyId((strtolower($dateNow->format('YF').'-'.$type))) as $sch_week)

                                    @foreach($supplies->findScheduleWeekId($sch_week->getId()) as $supply)

                                        @if($sch_week->getMId() == $category->getId())
                                            <div class="col-5">
                                                <span class="small">{{DateTime::createFromFormat('Y-m-d', $sch_week->getDate())->format('d M Y')}}</span>
                                            </div>

                                            @if($dateNow->format('Y-m-d') == $sch_week->getDate() && $sch_week->getIsDone() == null)
                                                <div class="col-6">
                                                    <a href="/admin/supply/{{$type}}/{{$sch_week->getId()}}/{{$supply->getId()}}/create">
                                            ...
                                            @else
                                                <div class="col-6">
                                                    <a class="link-primary"
                                                       href="/admin/supply/{{$type}}/{{$sch_week->getId()}}/{{$supply->getId()}}/view">
