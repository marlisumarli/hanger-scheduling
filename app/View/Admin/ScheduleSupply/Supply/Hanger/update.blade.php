    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
                <a href="/admin/supply/{{$model["type"]}}/{{$model["schedule"]}}/{{$model["supplyId"]}}/view">{{$dateTime->format('d/m/Y')}}</a>
            </li>
            <li aria-current="page" class="breadcrumb-item active">Ubah Laporan</li>
        </ol>
    </nav>
    <div class="mb-4">
        <h1>Update Laporan Supply {{strtoupper($model["type"])}}</h1>
    </div>
                ...
                <div class="card rounded-3 shadow-lg">
                    <div class="card-body">
                        ...
                        <input class="form-control" id="targetSet" min="100" name="target" required type="number"
                               value="{{$model['supply']->getTargetSet()}}">
                    </div>
                </div>
            </div>
            ...
            <div class="col-md">
                @foreach ($model['hangers'] as $hanger)
                    <div class="card shadow-lg rounded-3 mb-2">
                        ...
                        <div class="card-body">
                            <div class="row g-1">
                                @foreach($model['supply_lines'] as $supplyLine)

                                    @if($supplyLine->getHangerId() == $hanger->getId() && $supplyLine->getSupplyId() == $model['supply']->getId())
                                        <div class="col-4">
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <a href="/admin/supply/{{$model["type"]}}/{{$model["schedule"]}}/{{$model["supplyId"]}}/view"
