...
<?php $__env->startSection('content'); ?>
    ...
    <div class="row d-flex justify-content-center">
            ...
            <ul class="list-group">
                <li class="list-group-item list-group-item-secondary d-flex justify-content-between align-items-start">
                    <div class="ms-2 me-auto">
                        <span class="fw-bold">Type : <?php echo e(strtoupper($find_id->getId())); ?></span>
                    </div>
                ...
                <li class="list-group-item list-group-item-secondary d-flex justify-content-between align-items-start">
                    <div class="ms-2 me-auto">
                        <span class="fw-bold">Jumlah Item : <?php echo e($find_id->getQty()); ?></span>
                    </div>
                    <button class="btn btn-sm rounded bg-warning py-1 ms-auto shadow-lg"
                            data-bs-placement="top"
        ...
        <div class="col-xl-4 col-lg-5 col-md-6 col-sm-8 mb-3">
            <form class="card rounded-3 shadow-lg"
                  action="/admin/item/<?php echo e($find_id->getId()); ?>/hanger/update" method="post" autocomplete="off">
                ...
                <div class="card-body p-2">
                    ...
                    <table class="table text-center">
                        ...
                        <?php for($i = 0; $i < $find_id->getQty(); $i++): ?>
                            <tr>
                                <td>
                                    <input class="form-control p-0 text-center order" id="numberOrder"
                                           name="<?php echo e($i >= count($hangers) ? 'orderNumber[]' : 'updateOrderNumber[]'); ?>"
                                           placeholder="0" required title="" type="number"
                                           value="<?php echo e($i < count($hangers) ? $hangers[$i]->getOrderNumber() : ''); ?>"
                                           min="1">
                                </td>
                                <td>
                                    <input class="form-control p-0 text-center" id="name"
                                           name="<?php echo e($i >= count($hangers) ? 'hangerName[]' : 'updateName[]'); ?>"
                                           placeholder="..."
                                           required title="" type="text"
                                           value="<?php echo e($i < count($hangers) ? $hangers[$i]->getName() : ''); ?>"
                                           min="1">
                                </td>
                                <td>
                                    <input class="form-control p-0 text-center" id="qty"
                                           name="<?php echo e($i >= count($hangers) ? 'qty[]' : 'updateQty[]'); ?>"
                                           placeholder="00"
                                           required title="" type="number"
                                           value="<?php echo e($i < count($hangers) ? $hangers[$i]->getQty() : ''); ?>"
                                           min="1">
                                </td>
                                <td>
                ...
                <div class="card-footer text-end">
                    <?php if(count($hangers) < $find_id->getQty()): ?>
                        <button name="register" class="btn btn-primary" type="submit">
                            Submit
                        </button>
                    <?php elseif(count($hangers) >= $find_id->getQty()): ?>
    ...
    <div aria-hidden="true" aria-labelledby="updateTypeLabel" class="modal fade"
        ...
        <form action="/admin/item/<?php echo e($find_id->getId()); ?>/update" method="post" class="modal-dialog">
...
<?php echo $__env->make('Admin/Layout/main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\hanger-management-pt-indospray\app\View/Admin/ItemList/update.blade.php ENDPATH**/ ?>